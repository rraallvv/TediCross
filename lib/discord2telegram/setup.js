	...
	dcBot.on("message", async (message) => {
		...
		if (bridges !== undefined) {
			bridges.forEach(async (bridge) => {
				...
				bridge.telegram.lastUserId = message.author.id;
				
				// Check for attachments and pass them on
				message.attachments.forEach(async ({url}) => {
					try {
						let textToSend;
						if (bridge.discord.sendUsernames && bridge.telegram.showUsername) {
						...
						bridge.telegram.showUsername = false;
	...
	dcBot.on("messageUpdate", async (oldMessage, newMessage) => {
		...
		bridges.forEach(async (bridge) => {
			try {
				...
				const re = new RegExp(`^<b>${senderName}</b>\n`);
				if (bridge.discord.sendUsernames && re.test(processedMessage)) {
					textToSend = `<b>${senderName}</b>\n${processedMessage}`;
				} else {
					textToSend = processedMessage;
				}
