	...
	dcBot.on("message", async (message) => {
		...
		if (bridges !== undefined) {
			bridges.forEach(async (bridge) => {
				...
				bridge.lastUserId = message.author.id;
				...
				message.attachments.forEach(async ({url}) => {
					try {
						...
						if (bridge.logMessages) {
							logger.log(`${bridge.name}: Discord to Telegram: '${senderName}' attached url: ${url}`);
						}
				...
				message.embeds.forEach((embed) => {
					...
					if (bridge.logMessages) {
						logger.log(`${bridge.name}: Discord to Telegram: '${senderName}' embedded reply: '${text.replace(/\r?\n/g, "\\n")}'`);
					}
				...
				if (message.cleanContent) {
					...
					try {
						...
						if (bridge.logMessages) {
							logger.log(`${bridge.name}: Discord to Telegram: '${senderName}' sent message: '${processedMessage.replace(/\r?\n/g, "\\n")}'`);
						}
	...
	dcBot.on("messageUpdate", async (oldMessage, newMessage) => {
		...
		bridges.forEach(async (bridge) => {
			try {
				...
				if (bridge.logMessages) {
					logger.log(`${bridge.name}: Discord to Telegram from '${senderName}' edited message: '${processedMessage.replace(/\r?\n/g, "\\n")}'`);
				}
