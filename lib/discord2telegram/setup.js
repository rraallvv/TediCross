		...
		if (bridges !== undefined) {
			bridges.forEach(async (bridge) => {
				...
				message.attachments.forEach(async ({url}) => {
					try {
						...
						if (bridge.logMessages) {
							logger.log(`${bridge.name}: Discord to Telegram: '${senderName}' attached url:\n${url}`);
						}
				...
				message.embeds.forEach((embed) => {
					...
					if (bridge.logMessages) {
						logger.log(`${bridge.name}: Discord to Telegram: '${senderName}' embedded reply:\n${text}`);
					}
				...
				if (message.cleanContent) {
					...
					try {
						...
						if (bridge.logMessages) {
							logger.log(`${bridge.name}: Discord to Telegram: '${senderName}' sent message:\n${processedMessage}`);
						}
	...
	dcBot.on("messageUpdate", async (oldMessage, newMessage) => {
		...
		bridges.forEach(async (bridge) => {
			try {
				...
				if (bridge.logMessages) {
					logger.log(`${bridge.name}: Discord to Telegram from '${senderName}' edited message:\n${processedMessage}`);
				}
