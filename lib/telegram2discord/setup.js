...
function makeFileSender(logger, tgBot, dcBot, messageMap, settings) {
	...
	return async function (bridge, {message, fileId, fileName, caption = "", resolveExtension = false}) {
		...
		if (bridge.logMessages) {
			logger.log(`${bridge.name}: Telegram to Discord: ${messageObj.from} sent file:`);
			dcMessage.attachments.forEach((attachment) => console.log(attachment.url));
		}
...
function setup(logger, tgBot, dcBot, messageMap, bridgeMap, settings) {
	...
	updateEmitter.on("text", wrapFunction(async (message, bridge) => {
		...
		if (bridge.telegram.regexFilters) {
			for (var i = 0, l = bridge.telegram.regexFilters.length && !deleteMessage; i < l; i++) {
		...
		if (deleteMessage) {
			if (bridge.logMessages) {
				logger.log(`${bridge.name}: Telegram to Discord: message from '${messageObj.from}' was filtered:\n${messageObj.text}`);
				if (messageObj.reply !== null) {
					logger.log(`  In reply to '${messageObj.reply.author}' with content:\n${messageObj.reply.text}`);
				}
			}
		...
		try {
			...
			if (messageObj.reply !== null) {
				...
				if (bridge.logMessages) {
					logger.log(`${bridge.name}: Telegram to Discord: '${messageObj.from}' embedded reply from '${messageObj.reply.author}':\n${messageObj.reply.text}`);
				}
			...
			if (bridge.logMessages) {
				logger.log(`${bridge.name}: Telegram to Discord: '${messageObj.from}' sent message:\n${messageObj.text}`);
			}
	...
	updateEmitter.on("messageEdit", wrapFunction(async (tgMessage, bridge) => {
		...
		try {
			...
			if (bridge.logMessages) {
				logger.log(`${bridge.name}: Telegram to Discord: '${messageObj.from}' edited message:\n${messageObj.text}`);
			}
