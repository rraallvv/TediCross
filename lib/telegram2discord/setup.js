...
function makeFileSender(tgBot, dcBot, messageMap, settings) {
	...
	return async function (bridge, {message, fileId, fileName, caption = "", resolveExtension = false}) {
		...
		bridge.discord.lastUserId = message.from.id;

		// Make the text to send
		const messageObj = messageConverter(message, tgBot, settings, dcBot, bridge);

		let textToSend;
		if (bridge.telegram.sendUsernames && bridge.discord.showUsername) {
		...
		bridge.discord.showUsername = false;
	};
}
...
function setup(logger, tgBot, dcBot, messageMap, bridgeMap, settings) {
	...
	updateEmitter.on("text", wrapFunction(async (message, bridge) => {
		...
		if (bridge.telegram.regexFilters) {
			for (var i = 0, l = bridge.telegram.regexFilters.length; i < l; i++) {
		...
		try {
			...
			let messageText;
			if (!bridge.telegram.sendUsernames || messageObj.reply !== null || !bridge.discord.showUsername) {
