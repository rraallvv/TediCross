 ...
 * @private
 */
function makeFileSender(logger, tgBot, dcBot, messageMap, settings) {
	/**
	...
	return async function (bridge, {message, fileId, fileName, caption = "", resolveExtension = false}) {
		...
		// Update the last user ID to know whether we need to shown the user name for this message
		bridge.lastUserId = message.from.id;

		// Make the text to send
		const messageObj = messageConverter(message, tgBot, settings, dcBot, bridge);

		let textToSend;
		if (bridge.telegram.sendUsernames && bridge.showUsername) {
			textToSend = `**${messageObj.from}**:\n${caption}`;
		} else {
			textToSend = caption;
		}
		...
		if (bridge.logMessages) {
			logger.log(`${bridge.name}: Telegram to Discord: '${messageObj.from}' sent file(s): ${dcMessage.attachments.map((attachment) => attachment.url).join(", ")}`);
			if (caption) {
				logger.log(`${bridge.name}: Telegram to Discord: '${messageObj.from}' sent message: '${caption.replace(/\r?\n/g, "\\n")}'`);
			}
		}
...
function setup(logger, tgBot, dcBot, messageMap, bridgeMap, settings) {
	...
	const sendFile = makeFileSender(logger, tgBot, dcBot, messageMap, settings);
	...
	updateEmitter.on("text", wrapFunction(async (message, bridge) => {
		...
		// Skip sending this message if there is a match in list of regex filters
		let deleteMessage = false;
		if (bridge.telegram.regexFilters) {
			for (var i = 0, l = bridge.telegram.regexFilters.length; i < l && !deleteMessage; i++) {
				const re = new RegExp(bridge.telegram.regexFilters[i], "i");
				if (re.test(message.text)) {
					deleteMessage = true;
					break;
				}
				if (typeof message.entities !== "undefined") {
					for (var j = 0, m = message.entities.length; j < m; j++) {
						const entity = message.entities[j];
						if (typeof entity.url !== "undefined" && re.test(entity.url)) {
							deleteMessage = true;
							break;
						}
						if (typeof entity.user !== "undefined" && re.test(entity.user)) {
							deleteMessage = true;
							break;
						}
					}
		...
		if (deleteMessage) {
			if (bridge.logMessages) {
				logger.log(`${bridge.name}: Telegram to Discord: message from '${messageObj.from}' was filtered: '${messageObj.text.replace(/\r?\n/g, "\\n")}'${messageObj.reply !== null ? ` in reply to '${messageObj.reply.author}' with content: '${messageObj.reply.text.replace(/\r?\n/g, "\\n")}'` : ""}`);
			}
			tgBot.deleteMessage({chat_id: bridge.telegram.chatId, message_id: message.message_id});
			return;
		}
		...
		try {
			...
			if (messageObj.reply !== null) {
				...
				if (bridge.logMessages) {
					logger.log(`${bridge.name}: Telegram to Discord: '${messageObj.from}' embedded reply from '${messageObj.reply.author}': '${messageObj.reply.text.replace(/\r?\n/g, "\\n")}'`);
				}
			...
			if (bridge.logMessages) {
				logger.log(`${bridge.name}: Telegram to Discord: '${messageObj.from}' sent message: '${messageObj.text.replace(/\r?\n/g, "\\n")}'`);
			}
	...
	updateEmitter.on("messageEdit", wrapFunction(async (tgMessage, bridge) => {
		...
		try {
			...
			// Check if user name is required and already in the old message
			const re = new RegExp(`^\\*\\*${messageObj.from}\\*\\*\n`);
			if (bridge.telegram.sendUsernames && re.test(dcMessage.content)) {
				textToSend = `**${messageObj.from}**\n${messageObj.text}`;
			} else {
				textToSend = messageObj.text;
			}

			if (bridge.logMessages) {
				logger.log(`${bridge.name}: Telegram to Discord: '${messageObj.from}' edited message: '${messageObj.text.replace(/\r?\n/g, "\\n")}'`);
			}
